...
const EXPECTED_TITLE_NL = 'Hoe word je zelfstandige in België? Volledige gids 2026';
...
async function verifyDutchContent() {
  ...
  try {
    ...
    if (!resource) {
      console.error('❌ No Legal resource found in database');
    ...
    console.log(`✅ Legal resource found (ID: ${resource.id})\n`);
    ...
    console.log(`EN: ${resource.title_en ? '✅' : '❌'} ${resource.title_en?.substring(0, 70) || 'MISSING'}...`);
    console.log(`NL: ${resource.title_nl ? '✅' : '❌'} ${resource.title_nl?.substring(0, 70) || 'MISSING'}...`);
    
    if (resource.title_nl === EXPECTED_TITLE_NL) {
      console.log('✅ Dutch title matches expected value');
    } else if (resource.title_nl) {
      console.log('⚠️  Dutch title exists but differs from expected');
    }
    console.log('');
    ...
    console.log(`EN: ${resource.excerpt_en ? `✅ ${resource.excerpt_en.length} chars` : '❌ MISSING'}`);
    console.log(`NL: ${resource.excerpt_nl ? `✅ ${resource.excerpt_nl.length} chars` : '❌ MISSING'}`);
    console.log('');
    ...
    console.log(`  Structured:  ${roAnalysis.hasStructure ? '✅' : '❌'}`);
    ...
    console.log(`  Structured:  ${enAnalysis.hasStructure ? '✅' : '❌'}`);
    ...
    console.log(`  Structured:  ${nlAnalysis.hasStructure ? '✅' : '❌'}`);
    ...
    if (issues.length === 0) {
      ...
      console.log('⚠️  ISSUES FOUND - Migration may not be applied correctly\n');
